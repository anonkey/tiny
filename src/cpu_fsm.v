...
module cpu_fsm (
  ...
  register #(.N(8)) alu_result_reg (
    .o_Q(r_alu_result),
    .i_D(w_in_execute ? i_alu_result : r_alu_result),
    .i_clk(i_clk), .i_rst_n(i_rst_n), .i_en(1'b1)
  );

  register #(.N(8)) rs2_data_reg (
    .o_Q(r_rs2_data),
    .i_D(w_in_execute ? i_rs2_data : r_rs2_data),
    .i_clk(i_clk), .i_rst_n(i_rst_n), .i_en(1'b1)
  );

  dff is_load_ff (
    .o_Q(r_is_load), .o_Qn(),
    .i_D(w_in_execute ? i_is_load : r_is_load),
    .i_clk(i_clk), .i_rst_n(i_rst_n), .i_en(1'b1)
  );

  dff is_store_ff (
    .o_Q(r_is_store), .o_Qn(),
    .i_D(w_in_execute ? i_is_store : r_is_store),
    .i_clk(i_clk), .i_rst_n(i_rst_n), .i_en(1'b1)
  );
