...
async def test_field_extraction_i_type(dut):
  ...
  instr = enc_imm6(0b1001, 0b010, 0b100, 0b101010)
  dut.instr.value = instr
  await Timer(1, unit="ns")
...
async def test_field_extraction_l_type(dut):
  ...
  instr = enc_imm8(0b1010, 0b111, 0xA5)
  dut.instr.value = instr
  await Timer(1, unit="ns")
...
async def test_bez_fields(dut):
  ...
  instr = enc_imm8(0b1100, 0b101, 0x42)
  dut.instr.value = instr
  await Timer(1, unit="ns")
...
async def test_all_fields_all_opcodes(dut):
  ...
  for opcode in range(16):
    ...
    rs1_val = (opcode + 1) % 8
    rs2_val = (opcode + 2) % 8
    instr = enc_r(opcode, rd_val, rs1_val, rs2_val)
    dut.instr.value = instr
    await Timer(1, unit="ns")
...
@cocotb.test()
async def test_is_load_is_store(dut):
  """is_load should be 1 only for LOAD (1101), is_store only for STORE (1110)."""
  for opcode in range(16):
    dut.instr.value = opcode << 12
    await Timer(1, unit="ns")

    expected_load = 1 if opcode == 0b1101 else 0
    expected_store = 1 if opcode == 0b1110 else 0
    assert int(dut.is_load.value) == expected_load, (
      f"opcode {opcode:#06b}: is_load should be {expected_load}, got {int(dut.is_load.value)}"
    )
    assert int(dut.is_store.value) == expected_store, (
      f"opcode {opcode:#06b}: is_store should be {expected_store}, got {int(dut.is_store.value)}"
    )
