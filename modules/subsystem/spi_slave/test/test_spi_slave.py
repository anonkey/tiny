...
async def spi_send_8(dut, value):
    """Master sends 8 bits to slave over MOSI (MSB first). CS must already be low."""
...
async def spi_recv_8(dut):
    """Master clocks 8 bits out of slave over MISO. CS must already be low."""
    received = 0
    for _ in range(8):
        dut.sclk.value = 0
        await Timer(SPI_HALF_PERIOD, units="ns")
...
async def test_rx_multiple(dut):
    ...
    for val in [0xDE, 0xAD, 0xBE, 0xEF]:
        await spi_send_8(dut, val)
        # Wait for byte_done to propagate
        await Timer(SYS_CLK_PERIOD * 4, units="ns")
