#!/usr/bin/env python3
"""Generate KiCad schematics from Verilog modules.

Parses module headers (ports, parameters, submodule instantiations) and
produces .kicad_sch / .kicad_sym files with proper symbol shapes and net
connectivity.

Usage:
    python kicad_gen.py alu                    # flat schematic for alu
    python kicad_gen.py half_cpu               # flat schematic (default)
    python kicad_gen.py half_cpu --hier        # hierarchical (grouped sub-sheets)
    python kicad_gen.py half_cpu --sym-only    # symbol library only
    python kicad_gen.py half_cpu -o out/       # custom output directory
"""
...
def generate_flat_schematic(top_mod, sub_modules, lib_prefix="half_cpu"):
    ...
    lines = [
        ...
        '    (comment 1 "Auto-generated by kicad_gen")',
        '  )',
        '',
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate KiCad schematics from Verilog modules."
    )
    parser.add_argument("module", help="Module name (resolved via manager.json)")
    parser.add_argument("--hier", action="store_true", help="Generate hierarchical schematic (sub-sheets per category)")
    parser.add_argument("--sym-only", action="store_true", help="Only generate symbol library")
    parser.add_argument("--digitaljs", action="store_true", help="Generate DigitalJS JSON (CircuitVerse compatible)")
    parser.add_argument("-o", "--output", default="", help="Output directory (default: module's kicad/ subdir)")
    ...
    out_dir = args.output or os.path.join(info["path"], "kicad")
